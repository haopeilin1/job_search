...
def rule_l2_attr_verify(msg, attachments, ctx, kb_companies, kb_positions):
    ...
    VERIFY_EXCLUDES = [
        # 自我描述语境（assess/explore 背景）
        "我有", "我的", "具备", "具有", "拥有", "做过", "做过", "做过",
        # 评估语境（assess）
...
- 判定口诀：
  ...
  3. 若 query 同时包含自我背景+匹配度词+属性词（如"我有3年经验但JD要求精通分布式，我够格吗"）→ ASSESS（核心诉求是评估匹配度）
...
- 当规则全 MISS（strength=MISS）时，你应完全独立判断，不受规则约束
