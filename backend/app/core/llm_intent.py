...
class LLMRuleRegistry:
    ...
    def classify_all(self, message: str, attachments: list) -> list:
        """
...
def rule_l2_global_explore(msg, attachments, ctx, kb_companies, kb_positions):
    if attachments and len(attachments) > 0:
        return None
    RANGE_KWS = ["所有", "全局", "哪家", "排序", "推荐", "适合我", "投哪些", "帮我选", "对比几家"]
    ...
    if has_range:
        ...
        return LLMRuleResult(
            ...
            rule_name="L2-全局探索", trigger=hit, metadata={},
        )
    if "对比" in msg:
        ...
        return LLMRuleResult(
            intent=LLMIntentType.EXPLORE, strength=RuleStrength.STRONG,
            rule_name="L2-全局探索", trigger="对比", metadata={},
        )
    return None
...
def rule_l2_prepare(msg, attachments, ctx, kb_companies, kb_positions):
    if attachments and len(attachments) > 0:
        return None
    PREPARE_KWS = ["面试题", "模拟面试", "会问什么", "怎么准备", "刁钻", "押题"]
    if any(kw in msg for kw in PREPARE_KWS):
        hit = next(kw for kw in PREPARE_KWS if kw in msg)
        return LLMRuleResult(
            intent=LLMIntentType.PREPARE, strength=RuleStrength.STRONG,
...
def rule_l2_attr_verify(msg, attachments, ctx, kb_companies, kb_positions):
    ...
    if has_attr:
        ...
        return LLMRuleResult(
            ...
            rule_name="L2-属性核实", trigger=hit, metadata={},
        )
    return None


def rule_l2_manage(msg, attachments, ctx, kb_companies, kb_positions):
    MANAGE_KWS = ["上传简历", "更新JD", "删除", "清空", "我的简历库"]
    if any(kw in msg for kw in MANAGE_KWS):
        hit = next(kw for kw in MANAGE_KWS if kw in msg)
        return LLMRuleResult(
            intent=LLMIntentType.MANAGE, strength=RuleStrength.STRONG,
...
def rule_l3_referenced_explore(msg, attachments, ctx, kb_companies, kb_positions):
    ...
    if has_company and has_explore:
        ...
        return LLMRuleResult(
            ...
            metadata={"company": next(c for c in kb_companies if c in msg)},
        )
    return None
...
def rule_l3_referenced_assess(msg, attachments, ctx, kb_companies, kb_positions):
    if attachments and len(attachments) > 0:
        return None
    has_company = any(c in msg for c in kb_companies)
    MATCH_KWS = ["匹配度", "适合吗", "差距", "差多少", "契合", "够格"]
    has_match = any(kw in msg for kw in MATCH_KWS)
    if has_company and has_match:
...
def rule_l3_referenced_prepare(msg, attachments, ctx, kb_companies, kb_positions):
    has_entity = any(e in msg for e in kb_companies + kb_positions)
    PREPARE_KWS = ["面试", "题目", "模拟", "会问什么", "怎么准备", "刁钻", "押题"]
...
2. 【改写后查询】
   ...
   - 追问类型：{follow_up_type}（expand/extend/format/none）
...
4. 追问类型辅助：
   - expand：通常维持上轮意图，深入细节
   - extend：可能切换至VERIFY或EXPLORE，引入新维度
   - format：维持上轮意图，仅改变输出形式
...
class LLMFallbackClassifier:
    ...
    async def arbitrate(
        ...
        if not filtered:
            return MultiIntentResult(
                ...
                needs_clarification=True,
                clarification_reason="所有候选意图置信度均低于阈值，无法确定用户意图",
            )
        ...
        # 步骤5: 全局槽位池合并
        global_slots = self._merge_global_slots(resolved)
    ...
    def _merge_global_slots(self, candidates: list) -> dict:
        """合并所有候选意图的槽位，去重并保留"""
        global_slots = {}
        for c in candidates:
    ...
    def _check_clarification_need(self, candidates: list, global_slots: dict) -> tuple:
        ...
        high_cost = [c for c in candidates if c.execution_cost == "high"]
        if high_cost and not global_slots.get("resume_available"):
...
class LLMIntentRouter:
    ...
    async def route(
        ...
        # ① 规则引擎
        rule_result = self.rule_registry.classify(rewrite_result.rewritten_query, attachments)
        logger.info(
    ...
    async def route_multi(
        ...
        # ① 规则引擎（全量收集）
        rule_matches = self.rule_registry.classify_all(rewrite_result.rewritten_query, attachments)
        logger.info(
