...
这是最常见的误判来源，请严格按以下标准区分：

- EXPLORE = 用户想从"多个岗位"中做筛选、排序、推荐、对比、选择
  → 关键语义：存在范围词（哪些、哪个、所有、全部、几家、几个、有什么）或选择动作（筛、投、推、挑、看看有什么）
  → 典型句式："能投哪些"、"哪个岗最有用"、"帮我看看有什么Java岗"、"把所有岗位分析一遍"
  → 核心目的：得到一份岗位清单或排序结果

- VERIFY = 用户想了解"某个/某些具体岗位"的属性信息
  → 关键语义：询问具体属性（薪资、要求、技能、学历、加班、福利、怎么样、介绍）
  → 典型句式："需要什么技能"、"薪资多少"、"这个岗加班吗"、"具体要求是什么"
  → 核心目的：了解某个已知岗位的细节

判定口诀：
1. "有哪些/哪个适合/能投哪些/最有价值/有什么用" → EXPLORE
2. "需要什么/薪资多少/加班吗/怎么样/要求是什么" → VERIFY
3. "把所有/全部岗位...分析一遍" → EXPLORE（分析的是岗位集合，不是单个岗位属性）
4. "上面那个岗...具体要求是什么" → VERIFY（追问已知岗位的属性，不是探索新岗位）
5. 若 query 同时包含范围词和属性词（如"能投哪些，顺便说下薪资"）→ 多意图：EXPLORE + VERIFY
...
6. 【谨慎使用 CHAT，禁止滥用】CHAT 是兜底意图，但绝不能成为"逃避判断"的捷径：
   - 规则已识别出非 CHAT 意图时，你**有权**基于独立语义判断改判 CHAT，但必须有明确的闲聊/问候证据（如纯打招呼、无业务关键词），不能仅仅因为"槽位缺失"或"记忆为空"就改 CHAT
   - 槽位缺失 ≠ 意图是 CHAT。缺槽位时正确做法是：保持你判断的意图，标记 `needs_clarification=true`，生成澄清问题
   - 对意图判断不确定时，正确做法是：输出你判断的意图（哪怕 confidence 较低），标记 `needs_llm_fallback=true`，交给大模型兜底。**绝对不要**为了"安全"而强行改成 CHAT
   - 只有消息确实没有业务关键词（匹配、适合、薪资、要求、面试、上传等）、且明显是闲聊/问候时，才判 CHAT
...
【输出格式】
请先进行一步推理分析，再给出正式结论。推理过程能防止你在判断置信度时过于自信。

第一步【推理分析】：用1-2句话分析用户query的语义特征、关键词、上下文线索，说明你判断该意图和槽位的理由。不要只给结论，要展示思考过程。例如："用户说了'帮我看看有什么'，这是范围词，同时query中没有询问具体属性（如薪资、要求），因此意图应为EXPLORE而非VERIFY。"

第二步【正式输出】：严格JSON，不要markdown代码块：

{{
  "intent": "EXPLORE/ASSESS/VERIFY/PREPARE/MANAGE/CHAT",
  "confidence": 0.0-1.0,
  "reason": "意图判断依据 + 槽位抽取依据",
  "reasoning": "第一步的推理分析文本（必填）",
  "slots": {{
    "company": "...",
    "position": "...",
    ...
...
- 规则WEAK/MISS → 你有权独立判断，包括推翻规则和重新抽槽
- 无法确定 → confidence<0.75，标记 needs_llm_fallback=true，但不要为了"安全"而改成 CHAT
"""
...
【示例7：规则WEAK命中ASSESS + 槽位缺失 + 坚持业务意图】
规则参考：intent="ASSESS"，strength="WEAK"，rule_name="引用式评估(弱匹配词)"
工作记忆：（空）
压缩记忆：（空）
长期记忆：（空）
...
输出：
{{"intent":"ASSESS","confidence":0.55,"reason":"规则WEAK命中'分析'和'匹配度'，query包含明确业务关键词（匹配度），语义上用户是在请求评估分析，不是闲聊。三层记忆全空无法补全company和position，但这不是改判CHAT的理由。正确做法是保持ASSESS意图，标记缺失槽位并触发澄清","slots":{{"company":null,"position":null,"attributes":["匹配度"],"jd_source":"kb","resume_available":false,"search_keywords":"匹配度"}},"slot_sources":{{"company":null,"position":null,"attributes":"current_query","jd_source":"default","resume_available":"system_state","search_keywords":"current_query"}},"missing_slots":["company","position","resume_available"],"needs_clarification":true,"clarification_question":"您想分析哪个公司、哪个岗位的匹配度？","clarification_options":["从知识库选择已有JD","上传新的JD图片","先上传简历"],"rule_agreement":true,"context_driven":false,"needs_llm_fallback":false}}
"""
...
class SmallModelCalibrator:
    ...
    async def calibrate_multi(
        ...
        for rule_result in rule_matches:
            ...
            # 【关键】QueryRewriter 解析出的指代消解结果，必须注入到 candidate slots 中
            # 解决 explore→verify 多轮断层：规则只看当前 query，看不到历史推荐的岗位
    ...
    def _inject_resolved_references(self, candidate: IntentCandidate, rewrite_result: QueryRewriteResult, session: SessionMemory):
        ...
        # 1b. 【关键】resolved_references 中的值可能是 company+position 组合（如"百度 AI产品实习生"）
        ...
        if session and hasattr(session, "evidence_cache") and session.evidence_cache:
            ...
            if needs_company or needs_position:
                ...
                if candidate.slots.get("company") or candidate.slots.get("position"):
                    logger.info(
                        f"[SmallModelCalibrator] 指代消解槽位注入 | "
...
"""
        ...
        try:
            ...
            result = self._parse_single_intent(raw, rule_result, rewrite_result)
            return result
        except Exception as e:
    ...
    def _parse_single_intent(self, raw: str, rule_result: LLMRuleResult, rewrite_result: QueryRewriteResult) -> IntentCandidate:
        ...
        # NOTE: 后处理1已移除。校准器的独立语义判断应被尊重，不再强制回退到规则意图。
        # 早期硬编码补丁导致：校准器正确识别为chat的case被强制修正为verify，最终走向澄清。
        # 现在由arbitrate层综合置信度和语义一致性做最终决策。
        
        # 后处理2b（精细版）：防止校准器过度保守 fallback 到 CHAT
        # 仅当规则识别到非CHAT意图、且规则已提取到实体（company/position）时触发
        # 避免把"阿里巴巴的AI Agent产品经理匹配吗"这类明确业务query误判为闲聊
        rule_slots = rule_result.metadata or {}
        has_entity_debug = bool(
            rule_slots.get("company") or rule_slots.get("position") or
            rule_slots.get("__correct_company__") or rule_slots.get("__correct_position__")
        )
        logger.warning(
            f"[PARSE_DEBUG] rule_intent={rule_result.intent.value if rule_result.intent else 'None'} | "
            f"calibrated_intent={intent.value} | confidence={confidence:.2f} | "
            f"rule_slots={rule_slots} | has_entity={has_entity_debug}"
        )
        if (
            rule_result.intent
            ...
            if has_entity:
                logger.warning(
                    ...
                    f"calibrated=chat | 规则已提取实体({rule_slots.get('company') or rule_slots.get('position')}) | "
                    f"回退到规则意图"
                )
                intent = rule_result.intent
        ...
        parallel_map = {
            ...
            LLMIntentType.EXPLORE: True, LLMIntentType.ASSESS: True,
        }
...
直接输出澄清问题，不要加任何前缀："""
    ...
    async def arbitrate(
        ...
        if not filtered:
            # 分层兜底：检查校准器中是否有 CHAT 候选（即使 confidence < 0.5）
            # 如果有，说明模型判断为闲聊，返回 CHAT 而非澄清
        ...
        # 步骤6: 澄清判断（传入 session 用于从 evidence_cache 补全槽位）
...
class LLMIntentRouter:
    ...
    async def route_multi(
        ...
        # 让校准器基于 query 语义判断是否为闲聊，避免规则未命中时直接走向澄清
        ...
        if not has_non_miss:
            ...
            logger.info(
                f"[LLMIntentRouter-Multi] 规则全部MISS，强制创建虚拟CHAT候选送入校准器"
            )
        ...
        if calibrated and len(calibrated) <= 2:
            ...
            if all_fast:
                ...
                # 快速通道也需执行冲突消解（CHAT降级、MANAGE优先、ASSESS→PREPARE依赖）
                fast_candidates = self.fallback._resolve_conflicts(fast_candidates)
                ...
                needs_clarification, reason = self.fallback._check_clarification_need(fast_candidates, global_slots, session)
