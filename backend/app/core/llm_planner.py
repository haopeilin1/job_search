...
_TEMPORAL_KEYWORDS = [
    "最新", "最近", "新", "今年", "目前", "现在", "当下", "近期",
    "刚刚", "前不久", "还招吗", "还在招", "还开放", "截止",
...
【示例2：多意图并行（ASSESS + VERIFY）】
用户Query: "帮我看看字节算法岗匹配度，顺便问问薪资多少"
意图拓扑：
Layer 0: assess(0.92) | verify(0.88)      ← 同层并行

规划思路：
1. ASSESS 和 VERIFY 都需要检索字节算法岗的JD，检索参数相同 → 只生成一个 kb_retrieve（T0），两者共用
2. ASSESS 需要 match_analyze（T1），依赖 T0
3. VERIFY 无需额外工具，直接聚合 T0 的检索结果即可
4. aggregate（T2）同时依赖 T1（assess结果）和 T0（verify的检索结果）

{
  "planner_thought": "ASSESS和VERIFY共享同一检索参数，合并为一个kb_retrieve。ASSESS需要match_analyze，VERIFY直接使用检索结果。两者并行执行，aggregate聚合。",
  "tasks": [
    {
      "task_id": "T0",
      "task_type": "tool_call",
      "tool_name": "kb_retrieve",
      "description": "检索字节跳动算法岗JD（ASSESS和VERIFY共用）",
      "parameters": {"query": "{{global_slots.search_keywords}}", "company": "字节跳动", "top_k": 5},
      "dependencies": [],
      "fallback": {"action": "ask_user", "reason": "未找到相关JD"}
    },
    {
      "task_id": "T1",
      "task_type": "tool_call",
      "tool_name": "match_analyze",
      "description": "分析简历与字节算法岗匹配度（ASSESS）",
      "parameters": {
        "resume_text": "{{global_slots.resume_text}}",
        "jd_text": "{{T0.output.chunks}}",
        "company": "字节跳动",
      ...
      "dependencies": ["T0"],
      "fallback": {"action": "skip", "reason": "匹配分析失败", "default_params": {"score": 0}}
    },
    {
      "task_id": "T2",
      "task_type": "aggregate",
      "description": "聚合ASSESS匹配结果和VERIFY检索结果",
      "parameters": {
        "assess": "{{T1.output}}",
        "verify_kb": "{{T0.output.chunks}}"
      },
      "dependencies": ["T0", "T1"],
      "fallback": {"action": "abort", "reason": "聚合失败"}
    }
  ],
  "execution_strategy": {
    "parallel_groups": [["T0"], ["T1"], ["T2"]],
    "critical_path": ["T0", "T1", "T2"],
    "estimated_cost": "high"
  }
}

【示例3：多意图串行（ASSESS → PREPARE）】
用户Query: "帮我看看字节算法岗匹配度，再出几道面试题"
意图拓扑：
Layer 0: assess(0.92)
Layer 1: prepare(0.88)                    ← 依赖ASSESS结果

规划思路：
1. ASSESS 先执行：kb_retrieve → match_analyze
2. PREPARE 依赖 ASSESS 的 match_result，必须等 match_analyze 完成后执行
3. 若 match_analyze 失败，interview_gen 使用通用 focus_area 降级执行

{
  "planner_thought": "PREPARE依赖ASSESS的match_result，必须串行。先检索→匹配分析→面试题生成→聚合。",
  "tasks": [
    {
      "task_id": "T0",
      "task_type": "tool_call",
      "tool_name": "kb_retrieve",
      "description": "检索字节跳动算法岗JD",
      "parameters": {"query": "字节跳动 算法工程师", "top_k": 5},
      "dependencies": [],
      "fallback": {"action": "ask_user", "reason": "未找到相关JD"}
    },
    {
      ...
      "dependencies": ["T0"],
      "fallback": {"action": "skip", "reason": "匹配分析失败", "default_params": {"score": 0, "gaps": ["待补充"]}}
    },
    {
      "task_id": "T2",
      "task_type": "tool_call",
      ...
      "dependencies": ["T1"],
      "fallback": {"action": "skip", "reason": "使用通用题库", "default_params": {"focus_area": ["项目经验", "算法基础"]}}
    },
    {
      "task_id": "T3",
      "task_type": "aggregate",
      "description": "聚合匹配分析和面试题结果",
      "parameters": {
        "assess": "{{T1.output}}",
        "prepare": "{{T2.output}}"
      },
...
class TaskGraphPlanner:
    ...
    def _parse_graph(
        ...
        return TaskGraph(
            ...
            context={
                ...
                "search_keywords": getattr(rewrite_result, "search_keywords", "") if rewrite_result else "",
                "rewritten_query": getattr(rewrite_result, "rewritten_query", "") if rewrite_result else "",
                "source_preference": getattr(rewrite_result, "source_preference", "neutral") if rewrite_result else "neutral",
            },
    ...
    def _remove_deprecated_tools(self, graph: TaskGraph) -> TaskGraph:
        ...
        for t in graph.tasks:
            if t.tool_name in deprecated_tools:
                removed_ids.add(t.task_id)
                logger.warning(f"[TaskGraphPlanner] 过滤废弃工具任务: {t.task_id} ({t.tool_name})")
            else:
                new_tasks.append(t)

        if removed_ids:
            for t in new_tasks:
                t.dependencies = [d for d in t.dependencies if d not in removed_ids]
            graph.tasks = new_tasks
            # 重新计算 parallel_groups 和 critical_path
            graph.execution_strategy.parallel_groups = graph.compute_parallel_groups()
            graph.execution_strategy.critical_path = graph.compute_critical_path()

        return graph

    def _inject_external_search_for_temporal(self, graph: TaskGraph) -> TaskGraph:
        """涉及时效性查询或用户偏好外部信息时，自动注入 external_search 任务"""
        ctx = getattr(graph, "context", {}) or {}
        search_keywords = (ctx.get("global_slots") or {}).get("search_keywords", "")
        rewritten_query = ctx.get("rewritten_query", "")
        source_preference = ctx.get("source_preference", "neutral")

        # 从多个来源检测时效性
        is_temporal = (
            _has_temporal_keywords(search_keywords)
            or _has_temporal_keywords(rewritten_query)
        )
        # 用户明确偏好外部/实时信息时，也触发 external_search
        force_external = source_preference in ("prefer_external", "prefer_realtime")
        if not is_temporal and not force_external:
            return graph

        has_external = any(t.tool_name == "external_search" for t in graph.tasks)
        if has_external:
            return graph

        # 核心修复：检测用了 search_keywords 和 rewritten_query 两个来源，
        # 执行时也必须 fallback，避免 search_keywords 为空导致传空 query
        effective_query = search_keywords or rewritten_query
        if not effective_query:
            logger.warning("[TaskGraphPlanner] 触发外部搜索但 effective_query 为空，跳过注入")
            return graph

        kb_task_ids = [t.task_id for t in graph.tasks if t.tool_name == "kb_retrieve"]
        ...
        new_task = TaskNode(
            ...
            description="补充外部搜索（时效性查询自动触发）",
            parameters={"query": effective_query, "count": 5},
            dependencies=kb_task_ids if kb_task_ids else [],
            fallback=TaskFallback(action="skip", reason="外部搜索失败"),
        )
        new_task.params_hash = new_task.compute_params_hash()
        graph.add_task(new_task)

        # 更新 aggregate 的依赖，把 external_search 也加进去
        for t in graph.tasks:
            if t.task_type == "aggregate":
                if new_task_id not in t.dependencies:
        ...
        logger.info(f"[TaskGraphPlanner] 自动注入外部搜索 | query='{search_keywords}' | deps={kb_task_ids}")
        return graph
    ...
    def _fallback_graph(
        ...
        # 遍历所有候选意图（非仅 primary），为每个意图生成任务链
        match_analyze_task: Optional[TaskNode] = None
        ...
        for cand in multi_result.candidates:
            ...
            elif intent == LLMIntentType.MANAGE:
                add_task(
                    task_type="tool_call", tool_name="file_ops",
                    description="资料管理",
                    parameters={"operation": slots.get("operation", "list_jds")},
                    fallback=TaskFallback(action="abort", reason="资料管理操作失败"),
                )

            elif intent == LLMIntentType.CHAT:
                add_task(
