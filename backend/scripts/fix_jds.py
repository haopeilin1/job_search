"""
修复 parse_and_upload_jds.py 产生的解析错误，重新入库。
"""
...
def split_jds(raw: str) -> list:
    ...
    parts = re.split(company_pattern, text)
    parts = [p.strip() for p in parts if p.strip()]

    # 合并被错误分割的段落：如果某段以 "OPPO尊重并保护" 开头，说明是上一个OPPO JD的一部分
    merged = []
    for p in parts:
        if merged and p.startswith('OPPO尊重并保护'):
            merged[-1] += '\n\n' + p
        else:
            merged.append(p)
    return merged
...
def _extract_location(text: str) -> str:
    # 先找 "工作地点" 模式
    m = re.search(r'工作地点[：:]\s*([\u4e00-\u9fa5、，,\\/\s]+?)(?:\n|$)', text)
    if m:
        loc = m.group(1).strip().replace('、', '/').replace('，', '/').replace(',', '/')
        loc = loc.replace(' ', '').replace('市', '')
        return loc

    # 匹配 "北京 | 实习" 或 "北京 | 产品" 等
    m = re.search(r'([\u4e00-\u9fa5]{2,5})\s*\|\s*(?:实习|产品|校招|日常)', text)
    if m:
        loc = m.group(1).strip()
        if loc in ('北京市', '上海市', '深圳市', '杭州市', '广州市'):
            return loc.replace('市', '')
        if loc in ('北京', '上海', '深圳', '杭州', '广州', '成都', '武汉', '西安', '南京', '苏州'):
            return loc

    # 匹配 "上海、北京 | 校招"
    m = re.search(r'([\u4e00-\u9fa5]{2,5})、([\u4e00-\u9fa5]{2,5})\s*\|\s*(?:校招|实习|日常)', text)
    if m:
        return f"{m.group(1)}/{m.group(2)}"

    # 匹配 "实习北京市" 这种连在一起的情况
    m = re.search(r'实习(北京|上海|深圳|杭州|广州|成都|武汉|西安|南京|苏州)市?', text)
    if m:
        return m.group(1)
...
def _split_hard_soft(requirements: list) -> tuple:
    hard, soft = [], []
    strong_soft = ['优先', '加分', '更佳', '欢迎', '者优先', '优先；']
    for r in requirements:
        r_clean = r.strip()
        if not r_clean:
            continue
        is_soft = any(m in r_clean for m in strong_soft)
        if is_soft:
            soft.append(r_clean)
        else:
            hard.append(r_clean)
...
def parse_single_jd(text: str) -> dict:
    ...
    # 岗位名：去掉公司名前缀和末尾的编码
    title = re.sub(r'^(字节跳动|美团|MiniMax|OPPO|360|滴滴)[：:\*\s]*', '', first_line).strip('*').strip()
    title = re.sub(r'\s*[-(（][^)]*(?:J\d+|\d{4,})[)）]?$', '', title)  # 去掉末尾的编号
    title = title.strip()
    if not title or len(title) < 3:
        for l in lines[1:5]:
            l_clean = l.strip('*').strip()
            if '实习生' in l_clean or '产品经理' in l_clean:
                title = l_clean
                break
    ...
    # 职责 / 要求 分段
    desc_markers = {'职位描述', '岗位职责', '工作职责'}
    req_markers = {'职位要求', '任职要求', '任职资格', '职责要求', '任职要求:', '任职要求：'}
    ...
    for line in lines:
        ...
        if l_clean in desc_markers or l_clean.startswith('职位描述') or l_clean.startswith('岗位职责') or l_clean.startswith('工作职责'):
            in_desc = True
            in_req = False
            continue
        if l_clean in req_markers or l_clean.startswith('职位要求') or l_clean.startswith('任职要求') or l_clean.startswith('任职资格') or l_clean.startswith('职责要求'):
            in_desc = False
            in_req = True
            continue
        if l_clean in ('加分项', '加分', '加分项：', '加分：', '加分项\n'):
            in_desc = False
            in_req = False
            continue
    ...
    if not responsibilities and not requirements:
        ...
        if len(numbered) >= 4:
            mid = len(numbered) // 2
            responsibilities = _clean_lines(numbered[:mid])
            requirements = _clean_lines(numbered[mid:])
...
def main():
    ...
    # 删除之前错误添加的 id >= 24
    cleaned = [j for j in existing if j.get("id", 0) < 24]
    print(f"[Fix] 删除错误记录后剩余 {len(cleaned)} 条")

    max_id = max([j.get("id", 0) for j in cleaned], default=0)
    ...
    for idx, jd_text in enumerate(jd_texts, 1):
        ...
        print(f"  [{idx}] {record['company']} | {record['position']} | {record['location']} | hard={len(record['sections']['hard_requirements'])} soft={len(record['sections']['soft_requirements'])}")

    cleaned.extend(new_records)
    with open(DATA_FILE, "w", encoding="utf-8") as f:
        json.dump(cleaned, f, ensure_ascii=False, indent=2, default=lambda o: o.isoformat() if isinstance(o, datetime) else str(o))

    print(f"\n[Done] 已入库 {len(new_records)} 条 JD，当前库共 {len(cleaned)} 条")
